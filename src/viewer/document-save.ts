...
export function uint8ArrayToBase64(u8: Uint8Array): string {
  // Chunked apply avoids both the stack-overflow risk of a single
  // String.fromCharCode(...u8) spread and the high constant factor of
  // one-char-at-a-time concatenation.
  const CHARS_PER_CALL = 8192;
  let s = "";
  for (let i = 0; i < u8.length; i += CHARS_PER_CALL) {
    s += String.fromCharCode.apply(null, u8.subarray(i, i + CHARS_PER_CALL) as unknown as number[]);
  }
  return btoa(s);
}
